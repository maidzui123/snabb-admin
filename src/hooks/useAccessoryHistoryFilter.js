...
const useAccessoryHistoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newAccessoryHistorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newAccessoryHistorys.push(data[i].data);
    }
  };

  const handleUploadAccessoryHistorys = () => {
    if (newAccessoryHistorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      AccessoryHistoryServices.addAllAccessoryHistorys(newAccessoryHistorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        AccessoryHistoryServices.addAllAccessoryHistorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadAccessoryHistorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useAccessoryHistoryFilter;
