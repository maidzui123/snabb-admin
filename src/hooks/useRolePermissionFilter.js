...
const useRolePermissionFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newRolePermissions] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newRolePermissions.push(data[i].data);
    }
  };

  const handleUploadRolePermissions = () => {
    if (newRolePermissions.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      RolePermissionServices.addAllRolePermissions(newRolePermissions)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        RolePermissionServices.addAllRolePermissions(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadRolePermissions,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useRolePermissionFilter;
