...
const useBrandCategoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newBrandCategorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newBrandCategorys.push(data[i].data);
    }
  };

  const handleUploadBrandCategorys = () => {
    if (newBrandCategorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      BrandCategoryServices.addAllBrandCategorys(newBrandCategorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        BrandCategoryServices.addAllBrandCategorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadBrandCategorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useBrandCategoryFilter;
