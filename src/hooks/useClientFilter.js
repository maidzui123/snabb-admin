...
const useClientFilter = (data) => {
  ...
  const handleUploadClients = () => {
    if (newClients.length < 1) {
      ...
      ClientServices.addAllClients(newClients)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ClientServices.addAllClients(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
