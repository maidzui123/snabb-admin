...
const useUsersRoleFilter = (data) => {
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newUsersRoles.push(data[i].data);
    }
  };

  const handleUploadUsersRoles = () => {
    if (newUsersRoles.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      UsersRoleServices.addAllUsersRoles(newUsersRoles)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        UsersRoleServices.addAllUsersRoles(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadUsersRoles,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useUsersRoleFilter;
