...
const useRetailerCategoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newRetailerCategorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newRetailerCategorys.push(data[i].data);
    }
  };

  const handleUploadRetailerCategorys = () => {
    if (newRetailerCategorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      RetailerCategoryServices.addAllRetailerCategorys(newRetailerCategorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        RetailerCategoryServices.addAllRetailerCategorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadRetailerCategorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useRetailerCategoryFilter;
