...
const useAdminActivityFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newAdminActivitys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newAdminActivitys.push(data[i].data);
    }
  };

  const handleUploadAdminActivitys = () => {
    if (newAdminActivitys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      AdminActivityServices.addAllAdminActivitys(newAdminActivitys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        AdminActivityServices.addAllAdminActivitys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadAdminActivitys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useAdminActivityFilter;
