...
const useJobActivityFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newJobActivitys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newJobActivitys.push(data[i].data);
    }
  };

  const handleUploadJobActivitys = () => {
    if (newJobActivitys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      JobActivityServices.addAllJobActivitys(newJobActivitys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        JobActivityServices.addAllJobActivitys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadJobActivitys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useJobActivityFilter;
