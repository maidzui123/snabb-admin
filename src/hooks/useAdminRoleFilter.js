...
const useAdminRoleFilter = (data) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        AdminRoleServices.addAllAdminRoles(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadAdminRoles,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useAdminRoleFilter;
