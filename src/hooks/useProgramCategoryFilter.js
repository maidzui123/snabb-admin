...
const useProgramCategoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newProgramCategorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newProgramCategorys.push(data[i].data);
    }
  };

  const handleUploadProgramCategorys = () => {
    if (newProgramCategorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ProgramCategoryServices.addAllProgramCategorys(newProgramCategorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ProgramCategoryServices.addAllProgramCategorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadProgramCategorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useProgramCategoryFilter;
