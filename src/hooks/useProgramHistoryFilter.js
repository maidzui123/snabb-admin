...
const useProgramHistoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newProgramHistorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newProgramHistorys.push(data[i].data);
    }
  };

  const handleUploadProgramHistorys = () => {
    if (newProgramHistorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ProgramHistoryServices.addAllProgramHistorys(newProgramHistorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ProgramHistoryServices.addAllProgramHistorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadProgramHistorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useProgramHistoryFilter;
