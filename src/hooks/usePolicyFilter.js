...
const usePolicyFilter = (data) => {
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newPolicys.push(data[i].data);
    }
  };

  const handleUploadPolicys = () => {
    if (newPolicys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      PolicyServices.addAllPolicys(newPolicys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        PolicyServices.addAllPolicys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
...
export default usePolicyFilter;
