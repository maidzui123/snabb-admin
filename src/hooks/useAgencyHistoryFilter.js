...
const useAgencyHistoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newAgencyHistorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newAgencyHistorys.push(data[i].data);
    }
  };

  const handleUploadAgencyHistorys = () => {
    if (newAgencyHistorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      AgencyHistoryServices.addAllAgencyHistorys(newAgencyHistorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        AgencyHistoryServices.addAllAgencyHistorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadAgencyHistorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useAgencyHistoryFilter;
