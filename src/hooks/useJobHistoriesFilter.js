...
const useJobHistoriesFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newJobHistoriess] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newJobHistoriess.push(data[i].data);
    }
  };

  const handleUploadJobHistoriess = () => {
    if (newJobHistoriess.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      JobHistoriesServices.addAllJobHistoriess(newJobHistoriess)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        JobHistoriesServices.addAllJobHistoriess(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadJobHistoriess,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useJobHistoriesFilter;
