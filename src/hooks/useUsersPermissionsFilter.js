...
const useUsersPermissionsFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newUsersPermissionss] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newUsersPermissionss.push(data[i].data);
    }
  };

  const handleUploadUsersPermissionss = () => {
    if (newUsersPermissionss.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      UsersPermissionsServices.addAllUsersPermissionss(newUsersPermissionss)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        UsersPermissionsServices.addAllUsersPermissionss(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadUsersPermissionss,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useUsersPermissionsFilter;
