...
const useProgramFilter = (data) => {
  ...
  const handleUploadPrograms = () => {
    if (newPrograms.length < 1) {
      ...
      ProgramServices.addAllPrograms(newPrograms)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ProgramServices.addAllPrograms(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
