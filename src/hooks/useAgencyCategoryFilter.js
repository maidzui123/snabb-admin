...
const useAgencyCategoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newAgencyCategorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newAgencyCategorys.push(data[i].data);
    }
  };

  const handleUploadAgencyCategorys = () => {
    if (newAgencyCategorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      AgencyCategoryServices.addAllAgencyCategorys(newAgencyCategorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        AgencyCategoryServices.addAllAgencyCategorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadAgencyCategorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useAgencyCategoryFilter;
