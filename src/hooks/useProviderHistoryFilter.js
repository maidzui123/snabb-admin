...
const useProviderHistoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newProviderHistorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newProviderHistorys.push(data[i].data);
    }
  };

  const handleUploadProviderHistorys = () => {
    if (newProviderHistorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ProviderHistoryServices.addAllProviderHistorys(newProviderHistorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ProviderHistoryServices.addAllProviderHistorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadProviderHistorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useProviderHistoryFilter;
