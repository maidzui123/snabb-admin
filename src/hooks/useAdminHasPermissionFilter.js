...
const useAdminHasPermissionFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newAdminHasPermissions] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newAdminHasPermissions.push(data[i].data);
    }
  };

  const handleUploadAdminHasPermissions = () => {
    if (newAdminHasPermissions.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      AdminHasPermissionServices.addAllAdminHasPermissions(newAdminHasPermissions)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        AdminHasPermissionServices.addAllAdminHasPermissions(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadAdminHasPermissions,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useAdminHasPermissionFilter;
