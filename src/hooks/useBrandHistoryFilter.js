...
const useBrandHistoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newBrandHistorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newBrandHistorys.push(data[i].data);
    }
  };

  const handleUploadBrandHistorys = () => {
    if (newBrandHistorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      BrandHistoryServices.addAllBrandHistorys(newBrandHistorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        BrandHistoryServices.addAllBrandHistorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadBrandHistorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useBrandHistoryFilter;
