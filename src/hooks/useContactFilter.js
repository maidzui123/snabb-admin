...
const useContactFilter = (data) => {
  ...
  const handleUploadContacts = () => {
    if (newContacts.length < 1) {
      ...
      ContactServices.addAllContacts(newContacts)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ContactServices.addAllContacts(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
