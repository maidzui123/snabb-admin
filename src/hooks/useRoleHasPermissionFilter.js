...
const useRoleHasPermissionFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newRoleHasPermissions] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newRoleHasPermissions.push(data[i].data);
    }
  };

  const handleUploadRoleHasPermissions = () => {
    if (newRoleHasPermissions.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      RoleHasPermissionServices.addAllRoleHasPermissions(newRoleHasPermissions)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        RoleHasPermissionServices.addAllRoleHasPermissions(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadRoleHasPermissions,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useRoleHasPermissionFilter;
