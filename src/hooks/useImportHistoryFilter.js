...
const useImportHistoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newImportHistorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newImportHistorys.push(data[i].data);
    }
  };

  const handleUploadImportHistorys = () => {
    if (newImportHistorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ImportHistoryServices.addAllImportHistorys(newImportHistorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ImportHistoryServices.addAllImportHistorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadImportHistorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useImportHistoryFilter;
