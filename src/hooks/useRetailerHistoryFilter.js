...
const useRetailerHistoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newRetailerHistorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newRetailerHistorys.push(data[i].data);
    }
  };

  const handleUploadRetailerHistorys = () => {
    if (newRetailerHistorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      RetailerHistoryServices.addAllRetailerHistorys(newRetailerHistorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        RetailerHistoryServices.addAllRetailerHistorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadRetailerHistorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useRetailerHistoryFilter;
