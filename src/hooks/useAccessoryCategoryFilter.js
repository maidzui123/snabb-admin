...
const useAccessoryCategoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newAccessoryCategorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newAccessoryCategorys.push(data[i].data);
    }
  };

  const handleUploadAccessoryCategorys = () => {
    if (newAccessoryCategorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      AccessoryCategoryServices.addAllAccessoryCategorys(newAccessoryCategorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        AccessoryCategoryServices.addAllAccessoryCategorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadAccessoryCategorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useAccessoryCategoryFilter;
