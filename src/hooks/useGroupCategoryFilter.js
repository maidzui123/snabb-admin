...
const useGroupCategoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newGroupCategorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newGroupCategorys.push(data[i].data);
    }
  };

  const handleUploadGroupCategorys = () => {
    if (newGroupCategorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      GroupCategoryServices.addAllGroupCategorys(newGroupCategorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        GroupCategoryServices.addAllGroupCategorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadGroupCategorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useGroupCategoryFilter;
