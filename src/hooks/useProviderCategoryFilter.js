...
const useProviderCategoryFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newProviderCategorys] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newProviderCategorys.push(data[i].data);
    }
  };

  const handleUploadProviderCategorys = () => {
    if (newProviderCategorys.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ProviderCategoryServices.addAllProviderCategorys(newProviderCategorys)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ProviderCategoryServices.addAllProviderCategorys(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadProviderCategorys,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useProviderCategoryFilter;
