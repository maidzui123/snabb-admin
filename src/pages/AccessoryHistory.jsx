...
import AccessoryHistoryTable from "@/components/accessoryHistory/AccessoryHistoryTable";
import SelectCategory from "@/components/form/SelectCategory";
import MainDrawer from "@/components/drawer/MainDrawer";
import AccessoryHistoryDrawer from "@/components/drawer/AccessoryHistoryDrawer";
import CheckBox from "@/components/form/CheckBox";
import useAccessoryHistoryFilter from "@/hooks/useAccessoryHistoryFilter";
...
const AccessoryHistory = () => {
  ...
  const { data, loading } = useAsync(() =>
    AccessoryHistoryServices.getAll({
      page: currentPage,
      limit: limitData,
      // category: category,
      title: searchText,
  ...
  } = useAccessoryHistoryFilter(data?.data);

  return (
    <>
      <PageTitle>{t("AccessoryHistorys Management")}</PageTitle>
      <DeleteModal
        ids={allId}
        setIsCheck={setIsCheck}
        title={title}
        service={AccessoryHistoryServices}
      />
      <BulkActionDrawer ids={allId} title="AccessoryHistorys" />
      {!serviceData?.length && (
        <MainDrawer>
          <AccessoryHistoryDrawer id={serviceId} />
        </MainDrawer>
      )}
            ...
            <TableContainer className="mb-8 rounded-b-lg">
              <Table>
                <TableHeader>
                  <tr>
                    ...
                    <TableCell>{t("Accessory")}</TableCell>
              ...
              <TableFooter>
                <Pagination
                  ...
                  label="AccessoryHistory Page Navigation"
                />
              </TableFooter>
            </TableContainer>
          ) : (
            <NotFound title="Lịch sử linh kiện" />
          )}
        </div>
      </div>
    </>
