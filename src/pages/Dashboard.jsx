...
import ButtonDateRangePicker from "@/components/common/ButtonDateRangePicker";
...
const Dashboard = () => {
  ...
  const { currentPage, handleChangePage, lang, triggerRecallAPI } =
  ...
  // USING CODE
  const { data: dashboardNumber, loading: loadingDashboardNumber } = useAsync(
    () =>
      DashboardServices.getDashboardNumber({
        start_date: getValues("date_range")?.at(0)?.startDate,
        end_date: getValues("date_range")?.at(0)?.endDate,
      })
  );
  const {
    data: dashboardPolicyByProduct,
    loading: loadingDashboardPolicyByProduct,
  } = useAsync(() =>
    DashboardServices.getDashboardPolicyByProduct({
      start_date: getValues("date_range")?.at(0)?.startDate,
      end_date: getValues("date_range")?.at(0)?.endDate,
    })
  );
  ...
  //   dashboardOrderAmount?.ordersData?.filter((item) => {
    ...
    <>
      <PageTitle>
        <div className="flex justify-between">
          <div>{t("DashboardOverview")}</div>
          <div>
            <ButtonDateRangePicker
              control={control}
              onChangeDate={triggerRecallAPI}
            />
          </div>
        </div>
      </PageTitle>
      ...
      <div className="grid gap-4 mb-8 md:grid-cols-4 xl:grid-cols-4">
        <CardItemTwo
          mode={mode}
          title="Tổng số hợp đồng bảo hiểm"
          Icon={ImStack}
          className="text-white dark:text-green-100 bg-teal-500"
          number={dashboardNumber?.total_policy || 0}
          loading={loadingDashboardNumber}
        />

        <CardItemTwo
          mode={mode}
          title="Tổng số lượng bồi thường đã xử lý"
          Icon={FiCreditCard}
          className="text-white dark:text-orange-100 bg-orange-400"
          number={dashboardNumber?.total_job_completed || 0}
          loading={loadingDashboardNumber}
        />

        <CardItemTwo
          mode={mode}
          title="Số sự vụ yêu cầu bồi thường"
          Icon={FiFileText}
          className="text-white dark:text-green-100 bg-blue-500"
          number={dashboardNumber?.total_job || 0}
          loading={loadingDashboardNumber}
        />

        <CardItemTwo
          mode={mode}
          title="Chỉ số xử lý bồi thường"
          Icon={ImCreditCard}
          className="text-white dark:text-green-100 bg-green-500"
          number={dashboardNumber?.average_job || 0}
          loading={loadingDashboardNumber}
        />
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 my-8">
        <ChartCard
          mode={mode}
          loading={loadingDashboardPolicyByProduct}
          title={"Số lượng ca / Số lượng thay thế linh kiện"}
        >
          <LineChart data={dashboardPolicyByProduct?.data || []} />
        </ChartCard>

        <ChartCard
          mode={mode}
          loading={loadingDashboardPolicyByProduct}
          title={"Tỉ lệ hợp đồng theo sản phẩm"}
        >
          <PieChart data={dashboardPolicyByProduct?.data?.policies || []} />
        </ChartCard>
      </div>
      ...
      {/* {loadingRecentOrder ? (
        ...
        <NotFound title="đơn đặt hàng gần đây" />
      )} */}
    </>
  );
};
