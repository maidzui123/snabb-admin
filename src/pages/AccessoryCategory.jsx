...
import AccessoryCategoryTable from "@/components/accessoryCategory/AccessoryCategoryTable";
import SelectCategory from "@/components/form/SelectCategory";
import MainDrawer from "@/components/drawer/MainDrawer";
import AccessoryCategoryDrawer from "@/components/drawer/AccessoryCategoryDrawer";
import CheckBox from "@/components/form/CheckBox";
import useAccessoryCategoryFilter from "@/hooks/useAccessoryCategoryFilter";
...
const AccessoryCategory = () => {
  ...
  const { data, loading } = useAsync(() =>
    notifyPromise(
      AccessoryCategoryServices.getAll({
        ...getValues(),
        page: currentPage,
        limit: limitData,
        // category: category,
  ...
  } = useAccessoryCategoryFilter(data?.data);
  ...
  return (
    <>
      <PageTitle>{t("AccessoryCategorys Management")}</PageTitle>
      {isCheck?.length < 2 ? (
        <DeleteModal
          id={serviceId}
          setIsCheck={setIsCheck}
          title={title}
          service={AccessoryCategoryServices}
        />
      ) : (
        <DeleteModal
          ids={isCheck}
      ...
      <BulkActionDrawer ids={allId} title="AccessoryCategorys" />
      {!serviceData?.length && (
        <MainDrawer>
          <AccessoryCategoryDrawer id={serviceId} />
        </MainDrawer>
      )}
      <ToolbarForm
        data={data}
            ...
            <TableContainer className="mb-8 rounded-b-lg">
              ...
              <TableFooter>
                <Pagination
                  ...
                  label="AccessoryCategory Page Navigation"
                />
              </TableFooter>
            </TableContainer>
          ) : (
            <NotFound title="Phân loại linh kiện" />
          )}
        </div>
      </div>
    </>
