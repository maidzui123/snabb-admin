...
import RetailerHistoryTable from "@/components/retailerHistory/RetailerHistoryTable";
import SelectCategory from "@/components/form/SelectCategory";
import MainDrawer from "@/components/drawer/MainDrawer";
import RetailerHistoryDrawer from "@/components/drawer/RetailerHistoryDrawer";
import CheckBox from "@/components/form/CheckBox";
import useRetailerHistoryFilter from "@/hooks/useRetailerHistoryFilter";
...
const RetailerHistory = () => {
  ...
  const { data, loading } = useAsync(() =>
    RetailerHistoryServices.getAll({
      page: currentPage,
      limit: limitData,
      // category: category,
      title: searchText,
  ...
  } = useRetailerHistoryFilter(data?.data);

  return (
    <>
      <PageTitle>{t("RetailerHistorys Management")}</PageTitle>
      <DeleteModal
        ids={allId}
        setIsCheck={setIsCheck}
        title={title}
        service={RetailerHistoryServices}
      />
      <BulkActionDrawer ids={allId} title="RetailerHistorys" />
      {!serviceData?.length && (
        <MainDrawer>
          <RetailerHistoryDrawer id={serviceId} />
        </MainDrawer>
      )}
            ...
            <TableContainer className="mb-8 rounded-b-lg">
              ...
              <TableFooter>
                <Pagination
                  ...
                  label="RetailerHistory Page Navigation"
                />
              </TableFooter>
            </TableContainer>
          ) : (
            <NotFound title="RetailerHistory" />
          )}
        </div>
      </div>
    </>
