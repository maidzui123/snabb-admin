...
import RetailerCategoryTable from "@/components/retailerCategory/RetailerCategoryTable";
import SelectCategory from "@/components/form/SelectCategory";
import MainDrawer from "@/components/drawer/MainDrawer";
import RetailerCategoryDrawer from "@/components/drawer/RetailerCategoryDrawer";
import CheckBox from "@/components/form/CheckBox";
import useRetailerCategoryFilter from "@/hooks/useRetailerCategoryFilter";
...
const RetailerCategory = () => {
  ...
  const { data, loading } = useAsync(() =>
    notifyPromise(
      RetailerCategoryServices.getAll({
        ...getValues(),
        page: currentPage,
        limit: limitData,
        // category: category,
  ...
  } = useRetailerCategoryFilter(data?.data);
  ...
  return (
    <>
      <PageTitle>{t("RetailerCategorys Management")}</PageTitle>
      {isCheck?.length < 2 ? (
        <DeleteModal
          id={serviceId}
          setIsCheck={setIsCheck}
          title={title}
          service={RetailerCategoryServices}
        />
      ) : (
        <DeleteModal
          ids={isCheck}
      ...
      <BulkActionDrawer ids={allId} title="RetailerCategorys" />
      {!serviceData?.length && (
        <MainDrawer>
          <RetailerCategoryDrawer id={serviceId} />
        </MainDrawer>
      )}
            ...
            <TableContainer className="mb-8 rounded-b-lg">
              ...
              <TableFooter>
                <Pagination
                  ...
                  label="RetailerCategory Page Navigation"
                />
              </TableFooter>
            </TableContainer>
          ) : (
            <NotFound title="Phân loại TTBH " />
          )}
        </div>
      </div>
    </>
