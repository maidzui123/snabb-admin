...
const Log = () => {
  ...
  } = useLogFilter(data?.data);

  return (
    <>
      <PageTitle>{t("Logs Management")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title={title} service={ LogServices} />
      <BulkActionDrawer ids={allId} title="Logs" />
      {!serviceData?.length && <MainDrawer>
        <LogDrawer id={serviceId} />
      </MainDrawer>}
