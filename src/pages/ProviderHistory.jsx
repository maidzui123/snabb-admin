...
import ProviderHistoryTable from "@/components/providerHistory/ProviderHistoryTable";
import SelectCategory from "@/components/form/SelectCategory";
import MainDrawer from "@/components/drawer/MainDrawer";
import ProviderHistoryDrawer from "@/components/drawer/ProviderHistoryDrawer";
import CheckBox from "@/components/form/CheckBox";
import useProviderHistoryFilter from "@/hooks/useProviderHistoryFilter";
...
const ProviderHistory = () => {
  ...
  const { data, loading } = useAsync(() =>
    ProviderHistoryServices.getAll({
      page: currentPage,
      limit: limitData,
      // category: category,
      title: searchText,
  ...
  } = useProviderHistoryFilter(data?.data);

  return (
    <>
      <PageTitle>{t("ProviderHistorys Management")}</PageTitle>
      <DeleteModal
        ids={allId}
        setIsCheck={setIsCheck}
        title={title}
        service={ProviderHistoryServices}
      />
      <BulkActionDrawer ids={allId} title="ProviderHistorys" />
      {!serviceData?.length && (
        <MainDrawer>
          <ProviderHistoryDrawer id={serviceId} />
        </MainDrawer>
      )}
            ...
            <TableContainer className="mb-8 rounded-b-lg">
              ...
              <TableFooter>
                <Pagination
                  ...
                  label="ProviderHistory Page Navigation"
                />
              </TableFooter>
            </TableContainer>
          ) : (
            <NotFound title="ProviderHistory" />
          )}
        </div>
      </div>
    </>
