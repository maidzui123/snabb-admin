...
import ProviderCategoryTable from "@/components/providerCategory/ProviderCategoryTable";
import SelectCategory from "@/components/form/SelectCategory";
import MainDrawer from "@/components/drawer/MainDrawer";
import ProviderCategoryDrawer from "@/components/drawer/ProviderCategoryDrawer";
import CheckBox from "@/components/form/CheckBox";
import useProviderCategoryFilter from "@/hooks/useProviderCategoryFilter";
...
const ProviderCategory = () => {
  ...
  const { data, loading } = useAsync(() =>
    notifyPromise(
      ProviderCategoryServices.getAll({
        ...getValues(),
        page: currentPage,
        limit: limitData,
        // category: category,
  ...
  } = useProviderCategoryFilter(data?.data);
  ...
  return (
    <>
      <PageTitle>{t("ProviderCategorys Management")}</PageTitle>
      {isCheck?.length < 2 ? (
        <DeleteModal
          id={serviceId}
          setIsCheck={setIsCheck}
          title={title}
          service={ProviderCategoryServices}
        />
      ) : (
        <DeleteModal
          ids={isCheck}
      ...
      <BulkActionDrawer ids={allId} title="ProviderCategorys" />
      {!serviceData?.length && (
        <MainDrawer>
          <ProviderCategoryDrawer id={serviceId} />
        </MainDrawer>
      )}
            ...
            <TableContainer className="mb-8 rounded-b-lg">
              ...
              <TableFooter>
                <Pagination
                  ...
                  label="ProviderCategory Page Navigation"
                />
              </TableFooter>
            </TableContainer>
          ) : (
            <NotFound title="Phân loại NCC" />
          )}
        </div>
      </div>
    </>
