...
IMAGES_PROD="snabb_portal_auto:latest"
IMAGES_RELEASE="snabb_portal_auto:`date +%F_%H%M`"
IMAGES_AUTO="infall-api:portal"

ECR="646171399969.dkr.ecr.ap-southeast-1.amazonaws.com"

if [ "$1" == "auto" ] ; then
   echo "This is snabb-dev .. push Images to ECR Auto. So do it"
   docker tag  $IMAGES_PROD $ECR/$IMAGES_AUTO
   docker push             $ECR/$IMAGES_AUTO
...
docker push $ECR/$IMAGES_RELEASE
