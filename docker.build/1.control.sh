#!/bin/bash
echo ; echo ;
pwd="/home/docker.build/snabb-portal" ; cd $pwd ;
IMAGES_BLUE="snabb_portal_blue"
IMAGES_GREEN="snabb_portal_green"
IMAGES_CRON="snabb_portal_cron"
IMAGES_AUTO="infall-api:portal"
AWS_ECR="646171399969.dkr.ecr.ap-southeast-1.amazonaws.com"

blue="/etc/nginx/conf.d/blue_admin.conf"
green="/etc/nginx/conf.d/green_admin.conf"
...
doBuild () {
    ...
    if [ "$2" == "auto" ] ; then
       ...
       docker pull   $AWS_ECR/$IMAGES_AUTO
       #########################################
       #########################################
    else
